...
import {applyPatchToFile} from './patch-helpers.js';
import {applySmartEdits} from "./edit-helpers.js"; // Import the new patch helper
...
// --- MODIFIED EditFileArgsSchema ---
// Add the 'smart' mode and its specific parameters
const NormalizationOptionsSchema = z.object({
    ...
    .describe("Options for normalizing code before matching (used ONLY in 'smart' mode).");
...
const EditFileArgsSchema = z.object({
    ...
    // Add 'smart' to the enum of modes
    mode: z.enum(["exact", "structure", "semantic", "patch", "smart"]) // Added "smart"
        .optional()
        .default("smart") // Make "smart" the new default
        .describe("Editing mode: 'smart' (normalized/fuzzy matching), 'exact' (simple text replace), 'structure' (ignores whitespace/comments), 'patch' (apply unified diff), 'semantic' (future)."),

    // --- Fields for 'exact', 'structure', 'semantic', 'smart' ---
    edits: z.array(EditOperationSchema)
        .optional()
        .describe("List of edit operations (REQUIRED for 'exact', 'structure', 'semantic', 'smart' modes)."),

    // --- Fields ONLY for 'smart' mode ---
    normalizationOptions: NormalizationOptionsSchema, // Add schema here
    similarityThreshold: z.number().min(0).max(1).optional().default(1.0)
        .describe("Minimum similarity (0.0-1.0) for fuzzy matching (ONLY used in 'smart' mode). Default: 1.0."),
    ...
    // 'includeContext' might only be relevant for legacy modes now, keep if applyFileEdits uses it
    includeContext: z.boolean().optional().default(true).describe("Include context lines in the diff output for dry runs (primarily for non-smart/non-patch modes). Default: true"),

}).strict().superRefine((data, ctx) => {
    // Refined validation based on the selected mode
    if (data.mode === 'patch') {
        if (!data.patch) ctx.addIssue({
        ...
        if (data.normalizationOptions && Object.keys(data.normalizationOptions).length > 0) ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "'normalizationOptions' is only valid for 'smart' mode.",
            path: ["normalizationOptions"]
        });
        if (data.similarityThreshold !== 1.0) ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "'similarityThreshold' is only valid for 'smart' mode.",
            path: ["similarityThreshold"]
        });
        if (data.disambiguationIndex) ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "'disambiguationIndex' is only valid for 'smart' mode.",
            path: ["disambiguationIndex"]
        });
    } else if (data.mode === 'smart') {
        if (!data.edits || data.edits.length === 0) ctx.addIssue({
            code: z.ZodIssueCode.custom,
        ...
        // No need to check other fields like includeContext as they might be ignored or used internally
    } else { // Modes: 'exact', 'structure', 'semantic' (Legacy logic path)
        if (!data.edits || data.edits.length === 0) ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: `Input 'edits' is required and cannot be empty when mode is '${data.mode}'.`,
            path: ["edits"]
        });
        if (data.patch) ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: `Input 'patch' should not be provided when mode is '${data.mode}'.`,
            path: ["patch"]
        });
        if (data.normalizationOptions && Object.keys(data.normalizationOptions).length > 0) ctx.addIssue({
            code: z.ZodIssueCode.custom,
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            ...
            { // --- Use the MODIFIED schema for edit_file ---
                name: "edit_file",
                description:
                    "Edit file using 'smart' (normalized/fuzzy), 'patch' (unified diff), or legacy modes ('exact', 'structure'). Default: 'smart'.",
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        switch (name) {
            ...
            case "edit_file": {
                const parsedArgs = EditFileArgsSchema.parse(args ?? {}); // Validate against the *new* schema
                const validPath = await validatePath(parsedArgs.path);

                let result: string | object; // Result can be string or the structured object from applySmartEdits

                // --- Branch logic based on the mode ---
                if (parsedArgs.mode === 'patch') {
                    if (!parsedArgs.patch) throw new Error("Patch content is required for patch mode."); // Should be caught by refine, but good practice
                    ...
                    if (!parsedArgs.edits) throw new Error("Edits array is required for smart mode."); // Should be caught by refine
                    // Call the NEW smart edit function
                    result = await applySmartEdits(
                        validPath,
                        parsedArgs.edits,
                        parsedArgs.normalizationOptions, // Pass the options object
                        parsedArgs.similarityThreshold, // Pass threshold
                        parsedArgs.disambiguationIndex, // Pass optional index
                        parsedArgs.dryRun,
                        parsedArgs.formatAfter
                    );
                } else { // Legacy modes: 'exact', 'structure', 'semantic'
                    if (!parsedArgs.edits) throw new Error(`Edits array is required for ${parsedArgs.mode} mode.`); // Should be caught by refine
                    // Call the OLD applyFileEdits function
                    result = await applyFileEdits( // Use the original function here
                        validPath,
                        parsedArgs.edits,
                        parsedArgs.dryRun,
                        parsedArgs.mode, // Pass the legacy mode string
                        parsedArgs.formatAfter,
                        parsedArgs.includeContext // Pass includeContext for legacy modes
                    );
                }

                // --- Handle return structure ---
                if (typeof result === 'object' && result !== null && ('status' in result)) {
                    // Structured response (AMBIGUOUS, DRY_RUN_SUCCESS from applySmartEdits)
                    return {
                        content: [{type: "text", text: JSON.stringify(result)}]
                    };
                } else if (typeof result === 'string') {
                    // Simple success message or diff string from legacy/patch modes
                    return {
