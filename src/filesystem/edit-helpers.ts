...
export function normalizeCodeForMatching(
  ...
  const lines = normalizeLineEndings(originalText).split('\n');

  for (let i = 0; i < lines.length; i++) {
    let line = lines[i];
    const originalLineStartPos = currentOriginalPos;
    const originalLineEndPos = originalLineStartPos + line.length + 1; // +1 for newline
    ...
    if (ignoreComments) {
      // Simple removal; might fail with comments inside strings.
      commentStrippedLine = commentStrippedLine.replace(/\/\*[\s\S]*?\*\//g, '');
      commentStrippedLine = commentStrippedLine.replace(/\/\/.*$/, '');
    }
    ...
    if (ignoreInternalWhitespace === 'collapse') {
      whitespaceProcessedLine = whitespaceProcessedLine.replace(/\s+/g, ' ');
    } else if (ignoreInternalWhitespace === 'remove') {
      whitespaceProcessedLine = whitespaceProcessedLine.replace(/\s+/g, '');
    }
    // 'keep' does nothing here

    // 3. Handle Blank Lines
    const isEffectivelyBlank = whitespaceProcessedLine.trim() === '';
    if (ignoreBlankLines && isEffectivelyBlank) {
      currentOriginalPos = originalLineEndPos; // Advance past this original line
      continue; // Skip adding to normalized output
    }
    ...
    // Add mapping entry for this segment (maps original line to processed line)
    mapping.push({
      originalStart: originalLineStartPos,
      originalEnd: originalLineEndPos, // Exclusive end
      normalizedStart: normalizedLineStartPos,
      normalizedEnd: normalizedLineEndPos, // Exclusive end
    });

    // Advance original position
    currentOriginalPos = originalLineEndPos;
  }
...
export async function applyFileEdits(
  ...
  const appliedEditsInfo: Array<{ match: string, replacement: string, location: { line: number, column: number }}> = [];
